...
Set.prototype.eq = function (s) {
  ...
  for (let ele of this) if (!s.has(ele)) return false;
  return true;
}
...
const procNfa = ({ states, edges, terminals, dict }) => {
  const adjecent = ch => edges.filter(e => e.label === ch).reduce((p, c) =>
    (p[c.src].add(c.dest), p), states.reduce((p, i) => ((p[i] = new Set()), p), {}));
...
const nfa2dfa = (nfa) => {
  ...
  console.log(closure)
  const states = [closure[EPS][0]], newState = [closure[EPS][0]];
  const edges = [];
  ...
  console.log(states, edges, newState);
  return { edges, terminals: states.map((s, i) => ({ s, i })).filter(({s})=>terminals.some(t => s.has(t))).map(({ i })=>i), dict };
};
